...
(
  ...
  exec setsid /usr/local/sbin/tailscaled --no-logs-no-support 
) &
TAILSCALED_PID=$!
...
if [[ -n "$auth_key" ]]; then
  if [[ "$auth_key" == "test-auth-key" ]]; then
    ...
    /usr/local/bin/tailscale up --accept-routes=false --accept-dns=false --ssh --accept-risk=all --authkey="$auth_key" $hostnamearg ${TS_UP_CUSTOM_ARGS}
    /usr/local/bin/tailscale set --webclient=true
    /usr/local/bin/tailscale drive share remote_env /
    nohup bash -c 'code-server --host "$(tailscale ip -4)" --port 8080  --disable-telemetry --accept-server-license-terms --without-connection-token &'
  fi
fi
